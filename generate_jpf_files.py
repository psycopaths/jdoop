...
class GenerateConfFile:
    ...
    def generate_jpf_conf_file(self, input_file_name, output_file_name):
        ...
        with open(input_file_name, 'r') as f:
            ...
            output_file.write("\n")
            output_file.write("classpath+=,${jpf-jdart}/lib/junit4.jar\n")
            output_file.write("\n")
...
class CoordinateConfFileGeneration:
    def __init__(self, packagename, root, classes_to_analyze):
    ...
    def run(self):
        with open(self.location + self.classes_to_analyze, 'r') as f:
            for line_nl in f:
                class_name = line_nl[:-1]

                whole_path = self.location + class_name + ".java"

                # class_file = open(whole_path, 'r')

                g = GenerateConfFile('randooped')
                g.generate_jpf_conf_file(whole_path, self.location + class_name + ".jpf")
